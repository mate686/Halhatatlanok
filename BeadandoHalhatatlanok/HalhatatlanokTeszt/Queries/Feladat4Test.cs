...
namespace Halhatalanok.Test.Queries
{
    public class Feladat4Test
    {
        ...
        public void Feladat4_Teszt()
        {
            ...
            var kat = new Kategoriak { Nev = "színész" };

     
            for (int i = 1; i <= 9; i++)
                context.Tagok.Add(new Tagok { Nev = "Színész" + i, Ev = 2000, Kategoria = kat });

      
            for (int i = 1; i <= 3; i++)
                context.Tagok.Add(new Tagok { Nev = "X" + i, Ev = 1990, Kategoria = kat });
